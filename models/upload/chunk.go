...
type Chunk struct {
	...
	CurrentChunkSize int64 `orm:"column(current_chunk_size);pk;auto" json:"currentChunkSize"`
...
func (*ChunkModel) GetChunksByIdentifier(identifier string) (rst []Chunk, err error) {
	qb := models.MysqlBuilder().Select(" * ").From(" chunk T0 ")
	qb.Where(" 1 = 1 ")
	var params []interface{}
	qb.And(" T0.Identifier = ? ")
	params = append(params, identifier)
	qb.OrderBy(" T0.ChunkNumber ASC ")
	_, err = models.Ormer().Raw(qb.String(), params).QueryRows(&rst)
	if err != nil {
		return nil, err
	}
	return
